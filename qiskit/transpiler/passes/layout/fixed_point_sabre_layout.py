...
#
# Copy of ./sabre_layout.py further modified by Zachary Vernec
...
class FixedPointSabreLayout(TransformationPass):
    ...
    **References:**
    ...
    def run(self, dag):
        ...
        # Accept both keys for compatibility with existing SABRE pre-layout passes and callers.
        candidate_layouts = [
            *self.property_set.get("sabre_starting_layouts", []),
            *self.property_set.get("fixed_point_sabre_starting_layouts", []),
        ]
        starting_layouts = [to_partial_layout(layout) for layout in candidate_layouts]
        heuristic = (
