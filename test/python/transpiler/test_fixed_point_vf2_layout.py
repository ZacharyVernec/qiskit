...
from qiskit.transpiler.passes.layout.fixed_point_vf2_layout import (
    FixedPointVF2Layout,
    FixedPointVF2LayoutStopReason,
)
...
class LayoutTestCase(QiskitTestCase):
    """FixedPointVF2Layout assertions"""
    ...
    def assertLayout(self, dag, coupling_map, property_set, strict_direction=False):
        ...
        coupling_map"""
        self.assertEqual(
            property_set["FixedPointVF2Layout_stop_reason"],
...
@ddt.ddt
class TestFixedPointVF2LayoutSimple(LayoutTestCase):
    """Tests the FixedPointVF2Layout pass"""
    ...
    def test_2q_circuit_2q_coupling(self):
        ...
        pass_ = FixedPointVF2Layout(cmap, strict_direction=False, seed=self.seed, max_trials=1)
        pass_.run(dag)
    ...
    def test_2q_circuit_2q_coupling_sd(self):
        ...
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(cmap, strict_direction=True, seed=self.seed, max_trials=1)
        pass_.run(dag)
    ...
    def test_2q_circuit_simple_control_flow(self, strict_direction):
        ...
        with circuit.for_loop((1,)):
            circuit.cx(1, 0)
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(
            cmap, strict_direction=strict_direction, seed=self.seed, max_trials=1
        )
        pass_.run(dag)
    ...
    def test_3q_circuit_3q_coupling_non_induced(self):
        ...
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(cmap, seed=-1, max_trials=1)
        pass_.run(dag)
        self.assertLayout(dag, cmap, pass_.property_set)
    ...
    def test_call_limit(self):
        ...
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(cmap, seed=-1, call_limit=1)
        pass_.run(dag)
        self.assertEqual(
            pass_.property_set["FixedPointVF2Layout_stop_reason"],
            FixedPointVF2LayoutStopReason.NO_SOLUTION_FOUND,
        )
    ...
    def test_coupling_map_and_target(self):
        ...
        circuit.cx(qr[1], qr[0])  # qr1-> qr0
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(cmap, seed=-1, max_trials=1, target=target)
        pass_.run(dag)
    ...
    def test_neither_coupling_map_or_target(self):
        ...
        vf2_pass = FixedPointVF2Layout(seed=123, call_limit=1000, time_limit=20, max_trials=7)
        circuit = QuantumCircuit(2)
        dag = circuit_to_dag(circuit)
    ...
    def test_target_no_error(self):
        """Test that running fixed-point-vf2layout on a pass against a target with no error rates works."""
        n_qubits = 15
        target = Target()
        target.add_instruction(CXGate(), {(i, i + 1): None for i in range(n_qubits - 1)})
        vf2_pass = FixedPointVF2Layout(target=target)
        circuit = QuantumCircuit(2)
        circuit.cx(0, 1)
        dag = circuit_to_dag(circuit)
        vf2_pass.run(dag)
    ...
    def test_target_some_error(self):
        """Test that running fixed-point-vf2layout on a pass against a target with some error rates works."""
        n_qubits = 15
        target = Target()
        target.add_instruction(
    ...
    def test_determinism_all_1q(self):
        """Test that running fixed-point-vf2layout on a circuit with all single qubit gates is deterministic."""
        ...
        layouts = []
        for _ in range(10):
            layout_pass = FixedPointVF2Layout(target=backend.target)
            property_set = {}
    ...
    def test_complete_layout_with_idle_qubits(self, seed, strict_direction):
        ...
        property_set = {}
        pass_ = FixedPointVF2Layout(target=target, seed=seed, strict_direction=strict_direction)
...
@ddt.ddt
class TestFixedPointVF2LayoutLattice(LayoutTestCase):
    ...
    def test_hexagonal_lattice_graph_20_in_25(self):
        ...
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(self.cmap25, seed=self.seed, max_trials=1)
        pass_.run(dag)
    ...
    def test_hexagonal_lattice_graph_9_in_25_no_trial_limit(self, strict_direction):
        ...
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(
            self.cmap25, seed=-1, max_trials=-1, strict_direction=strict_direction
        )
        pass_.run(dag)
        self.assertLayout(dag, self.cmap25, pass_.property_set)
    ...
    def test_hexagonal_lattice_graph_9_in_25_default_trial_limit(self, strict_direction):
        ...
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(
            self.cmap25, seed=-1, max_trials=None, strict_direction=strict_direction
        )
        pass_.run(dag)
        self.assertLayout(dag, self.cmap25, pass_.property_set)


class TestFixedPointVF2LayoutBackend(LayoutTestCase):
    """Tests FixedPointVF2Layout against backends"""
    ...
    def test_5q_circuit_Rueschlikon_no_solution(self):
        ...
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(CouplingMap(cmap16), seed=self.seed, max_trials=1)
        pass_.run(dag)
    ...
    def test_9q_circuit_Rueschlikon_sd(self):
        ...
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(cmap16, strict_direction=True, seed=self.seed, max_trials=1)
        pass_.run(dag)
    ...
    def test_4q_circuit_Tenerife_loose_nodes(self):
        ...
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(cmap5, seed=self.seed, max_trials=1)
        pass_.run(dag)
    ...
    def test_3q_circuit_Tenerife_sd(self):
        ...
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(cmap5, strict_direction=True, seed=self.seed, max_trials=1)
        pass_.run(dag)
    ...
    def test_9q_circuit_Rueschlikon(self):
        ...
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(cmap16, strict_direction=False, seed=self.seed, max_trials=1)
        pass_.run(dag)
        self.assertLayout(dag, cmap16, pass_.property_set)
    ...
    def test_3q_circuit_Tenerife(self):
        ...
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(cmap5, strict_direction=False, seed=self.seed, max_trials=1)
        pass_.run(dag)
        self.assertLayout(dag, cmap5, pass_.property_set)
    ...
    def test_3q_circuit_vigo_with_custom_scores(self):
        ...
        vf2_pass = FixedPointVF2Layout(target=target, seed=1234568942)
        property_set = {}
        vf2_pass(circuit, property_set)
        pm = PassManager([FakeScore(), FixedPointVF2Layout(target=target, seed=1234568942)])
        pm.run(circuit)
    ...
    def test_perfect_fit_Manhattan(self):
        ...
        dag = circuit_to_dag(circuit)
        pass_ = FixedPointVF2Layout(cmap65, seed=self.seed, max_trials=1)
        pass_.run(dag)
        self.assertLayout(dag, cmap65, pass_.property_set)


class TestFixedPointVF2LayoutOther(LayoutTestCase):
    """Other FixedPointVF2Layout tests"""
    ...
    def test_seed(self):
        ...
        pass_1 = FixedPointVF2Layout(CouplingMap(cmap5), seed=seed_1, max_trials=1)
        pass_1.run(dag)
        layout_1 = pass_1.property_set["layout"]
        self.assertEqual(
            pass_1.property_set["FixedPointVF2Layout_stop_reason"],
            FixedPointVF2LayoutStopReason.SOLUTION_FOUND,
        )

        pass_2 = FixedPointVF2Layout(CouplingMap(cmap5), seed=seed_2, max_trials=1)
        pass_2.run(dag)
        layout_2 = pass_2.property_set["layout"]
        self.assertEqual(
            pass_2.property_set["FixedPointVF2Layout_stop_reason"],
            FixedPointVF2LayoutStopReason.SOLUTION_FOUND,
        )
    ...
    def test_3_q_gate(self):
        ...
        self.assertEqual(
            pass_1.property_set["FixedPointVF2Layout_stop_reason"],
            FixedPointVF2LayoutStopReason.MORE_THAN_2Q,
        )
...
class TestMultipleTrials(QiskitTestCase):
    ...
    def test_with_properties(self):
        ...
        vf2_pass = FixedPointVF2Layout(target=backend.target)
        property_set = {}
        vf2_pass(qc, property_set)
    ...
    def test_max_trials_exceeded(self):
        ...
        vf2_pass = FixedPointVF2Layout(target=backend.target, seed=-1, max_trials=1)
        property_set = {}
        vf2_pass(qc, property_set)
    ...
    def test_time_limit_exceeded(self):
        ...
        cmap = CouplingMap(YORKTOWN_CMAP)
        backend = GenericBackendV2(num_qubits=5, coupling_map=cmap, seed=1)
        vf2_pass = FixedPointVF2Layout(target=backend.target, seed=-1, time_limit=0.0)
        property_set = {}
        vf2_pass(qc, property_set)
        self.assertEqual(set(property_set["layout"].get_physical_bits()), {2, 0})
    ...
    def test_reasonable_limits_for_simple_layouts(self):
        ...
        vf2_pass = FixedPointVF2Layout(target=backend.target, seed=42)
        property_set = {}
        vf2_pass(qc, property_set)
