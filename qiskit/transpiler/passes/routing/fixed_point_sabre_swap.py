...
#
# Copy of ./sabre_swap.py further modified by Zachary Vernec
...
class FixedPointSabreSwap(TransformationPass):
    ...
    **References:**
    ...
    def __getstate__(self):
        # The Rust extension object does not have stable cross-module pickling semantics, so rebuild
        # it from the Python target after unpickling instead of serializing it directly.
        state = self.__dict__.copy()
        state["_had_routing_target"] = self._routing_target is not None
        state["_routing_target"] = None
        state.pop("dist_matrix", None)
        state.pop("coupling_map", None)
        return state

    def __setstate__(self, state):
        had_routing_target = state.pop("_had_routing_target", False)
        self.__dict__.update(state)
        if had_routing_target and self.target is not None:
            self._routing_target = RoutingTarget.from_target(self.target)
        self.__dict__.pop("dist_matrix", None)
        self.__dict__.pop("coupling_map", None)
