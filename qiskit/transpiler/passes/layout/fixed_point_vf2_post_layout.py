...
#
# Copied from ../vf2_post_layout.py
# Zachary Vernec modified this file to create a fixed-point VF2 variant


"""FixedPointVF2PostLayout pass to find a layout after transpile using subgraph isomorphism"""
...
class FixedPointVF2PostLayoutStopReason(Enum):
    """Stop reasons for FixedPointVF2PostLayout pass."""
...
class FixedPointVF2PostLayout(AnalysisPass):
    ...
    Unlike the :class:`~.FixedPointVF2Layout` transpiler pass which is designed to find an
    ...
    set in ``property_set['FixedPointVF2PostLayout_stop_reason']`` in all the cases and will be
    one of the values enumerated in ``FixedPointVF2PostLayoutStopReason`` which has the
    following values:
        ...
        * ``">2q gates in basis"``: If FixedPointVF2PostLayout can't work with the basis of the circuit.
    ...
    ``vf2_avg_error_map`` key in the property set when :class:`~.FixedPointVF2PostLayout`
    is run.
    """
    ...
    def __init__(
        ...
        Args:
            target (Target): A target representing the backend device to run ``FixedPointVF2PostLayout`` on.
            seed (int): Sets the seed of the PRNG. -1 Means no node shuffling.
            call_limit (int): The number of state visits to attempt in each execution of
                VF2.
            time_limit (float): The total time limit in seconds to run ``FixedPointVF2PostLayout``
            strict_direction (bool): Whether the pass is configured to follow
                ...
                :class:`~.DAGCircuit` object to :meth:`~.FixedPointVF2PostLayout.run` to be in
