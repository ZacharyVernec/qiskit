...
#
# Copy of ./test_sabre_swap.py further modified by Zachary Vernec

"""Test the FixedPointSabre Swap pass"""
...
from qiskit.transpiler.passes import FixedPointSabreSwap, CheckMap
from qiskit.transpiler.passes.routing.fixed_point_sabre_swap import Heuristic, SetScaling
...
def looping_circuit(uphill_swaps=1, additional_local_minimum_gates=0):
    """A circuit that causes FixedPointSabreSwap to loop infinitely for the legacy 'lookahead' heuristic.
...
@ddt.ddt
class TestFixedPointSabreSwap(QiskitTestCase):
    """Tests the FixedPointSabreSwap pass."""
    ...
    def test_sabre_swap_pickle(self):
        ...
        sabre_swap = FixedPointSabreSwap(target, "lookahead", seed=42, trials=1024)
        with io.BytesIO() as buf:
            pickle.dump(sabre_swap, buf)
            buf.seek(0)
            output = pickle.load(buf)
        self.assertIsInstance(output, FixedPointSabreSwap)
    ...
    def test_trivial_case(self):
        ...
        passmanager = PassManager(FixedPointSabreSwap(coupling, "basic"))
    ...
    def test_trivial_with_target(self):
        ...
        passmanager = PassManager(FixedPointSabreSwap(target, "basic"))
        new_qc = passmanager.run(qc)
    ...
    def test_lookahead_mode(self):
        ...
        pm = PassManager(FixedPointSabreSwap(coupling, "lookahead"))
        new_qc = pm.run(qc)
    ...
    def test_do_not_change_cm(self):
        ...
        passmanager = PassManager(FixedPointSabreSwap(coupling))
    ...
    def test_do_not_reorder_measurements(self):
        """Test that FixedPointSabreSwap doesn't reorder measurements to the same classical bit.
    ...
    def test_no_infinite_loop(self):
        """Test that the 'release value' mechanisms allow FixedPointSabreSwap to make progress even on
        ...
        # original FixedPointSabre paper.  We use this here because Qiskit's modern default heuristics don't
        ...
        routing_pass = PassManager(FixedPointSabreSwap(coupling_map, heuristic))
    ...
    def test_deterministic(self, heuristic):
        """Test that the output of the FixedPointSabreSwap pass is deterministic for a given random seed."""
        width = 40
        ...
        pass_0 = FixedPointSabreSwap(coupling, heuristic, seed=0, trials=1)
        pass_1 = FixedPointSabreSwap(coupling, heuristic, seed=1, trials=1)
        ...
        # degenerate circuit, then the test probably needs fixing (or FixedPointSabre is ignoring the seed).
    ...
    def test_rejects_too_many_qubits(self):
        ...
        number of qubits."""
        pass_ = FixedPointSabreSwap(CouplingMap.from_line(4))
...
@ddt.ddt
class TestFixedPointSabreSwapControlFlow(QiskitTestCase):
    ...
    def test_shared_block(self):
        ...
        cdag = FixedPointSabreSwap(coupling, "lookahead", seed=82, trials=1).run(
            circuit_to_dag(outer)
        )
    ...
    def test_blocks_use_registers(self):
        ...
        cdag = FixedPointSabreSwap(coupling, "lookahead", seed=82, trials=1).run(circuit_to_dag(qc))
    ...
    def test_pre_if_else_route(self):
        ...
        dag = circuit_to_dag(qc)
        cdag = FixedPointSabreSwap(coupling, "lookahead", seed=82, trials=1).run(dag)
        check_map_pass = CheckMap(coupling)
        check_map_pass.run(cdag)
        self.assertTrue(check_map_pass.property_set["is_swap_mapped"])
    ...
    def test_if_expr(self):
        ...
        dag = circuit_to_dag(qc)
        cdag = FixedPointSabreSwap(coupling, "lookahead", seed=58, trials=1).run(dag)
        check_map_pass = CheckMap(coupling)
        check_map_pass.run(cdag)
        self.assertTrue(check_map_pass.property_set["is_swap_mapped"])
    ...
    def test_switch_implicit_carg_use(self):
        ...
        fulfilled correctly."""
        coupling = CouplingMap.from_line(4)
        pass_ = FixedPointSabreSwap(coupling, "lookahead", seed=82, trials=1)
    ...
    def test_disjoint_looping(self):
        ...
        cqc = FixedPointSabreSwap(cm, "lookahead", seed=82, trials=1)(qc)
    ...
    def test_multiple_ops_per_layer(self):
        ...
        with qc.for_loop((0,)):
            qc.cx(3, 5)
        cqc = FixedPointSabreSwap(coupling, "lookahead", seed=82, trials=1)(qc)
        check_map_pass(cqc)
        self.assertTrue(check_map_pass.property_set["is_swap_mapped"])
    ...
    def test_idle_qubit_contraction(self):
        ...
        pass_ = FixedPointSabreSwap(coupling, "decay", seed=2025_02_05, trials=1)
        self.assertEqual(pass_(qc), expected)

    def test_nested_vars(self):
        """The FixedPointSabre rebuilder shouldn't choke if there is `Var` or `Stretch` usage within a
        control-flow block."""
        qc = QuantumCircuit(4)
        ...
        out = FixedPointSabreSwap(CouplingMap.from_line(4), heuristic="basic", seed=0, trials=1)(qc)
...
@ddt.ddt
class TestFixedPointSabreSwapRandomCircuitValidOutput(QiskitTestCase):
