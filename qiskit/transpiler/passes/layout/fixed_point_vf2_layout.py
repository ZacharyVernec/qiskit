...
#
# Copied from ../vf2_layout.py
# Zachary Vernec modified this file to create a fixed-point VF2 variant


"""FixedPointVF2Layout pass to find a layout using subgraph isomorphism"""
...
class FixedPointVF2LayoutStopReason(Enum):
    """Stop reasons for FixedPointVF2Layout pass."""
...
class FixedPointVF2Layout(AnalysisPass):
    ...
    set in ``property_set['FixedPointVF2Layout_stop_reason']`` in all the cases and will be
    one of the values enumerated in ``FixedPointVF2LayoutStopReason`` which has the
    following values:

        * ``"solution found"``: If a perfect layout was found.
        * ``"nonexistent solution"``: If no perfect layout was found.
        * ``">2q gates in basis"``: If FixedPointVF2Layout can't work with basis
    ...
    ``vf2_avg_error_map`` key in the property set when :class:`~.FixedPointVF2Layout` is run.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            time_limit (float): The total time limit in seconds to run ``FixedPointVF2Layout``.  This is not
            ...
            target (Target): A target representing the backend device to run ``FixedPointVF2Layout`` on.
